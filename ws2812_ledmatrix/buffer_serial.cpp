...
/******** User Setting Value **************/
byte stnNum = 4;          //PLC 국번
/******************************************/
word msgLen = 0;
byte sndBuf[MAX_SND] = {0};
byte rcvBuf[MAX_RCV] = {0}; 
byte dataBuf[64] = {0};
...
void BufferSerial::cmd_process(char cmd, char* data, int len)
{
  Serial.printf("cmd : %d \n", cmd);
  for (int i = 0; i < len; i++) {
    ...
    if (serHead != serTail) 
    {
      ...
      if (serRxLen)
      {
        ...
        if (recv_end == true) 
        {
          ...
          {
            cmd = packet[0];
            cmd_process(cmd, &packet[1], dataLen-1); // -1 is command
          }
          else {
            //send NACK
            buf[0] = NCK;
...
boolean BufferSerial::recv_callback()
{
  while(1)
  {
